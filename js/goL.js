var goL = (function(){
	...
	goL.editMode = function(f){
		editMode=f;
		f? rects.classed("edit",true) : rects.classed("edit",false);
	}
	...
	goL.clearGrid();
	var rects = d3.select("#gol-canvas")
        .selectAll("rect")
        .data(d3.range(0,bits)).enter()
            .append("rect")
            .classed("gol-rect",true)
...
d3.select(".gol-select").selectAll("option").data(d3.keys(games))
